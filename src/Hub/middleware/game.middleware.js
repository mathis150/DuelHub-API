...
export const namegamecheck = (req,res,next) =>  {
    if(!req.params.name_game) next({code:400,status:"name given for the query is invalid"})
    next()
}

export const namegenrecheck = (req,res,next) =>  {
    if(!req.params.name_genre) next({code:400,status:"genre given for the query is invalid"})
    next()
}

export const namestudiocheck = (req,res,next) =>  {
    if(!req.params.name_studio) next({code:400,status:"studio given for the query is invalid"})
    next()
}